...
const Form: React.FC = () => {
  ...
  useEffect(() => {
    const handleExternalSelect = (event: any) => {
      const { type, isNonprofit } = event.detail;
      if (type) {
        setFormData(prev => ({ 
          ...prev, 
          bookingType: type, 
    ...
    window.addEventListener('select-booking-type', handleExternalSelect);
    return () => window.removeEventListener('select-booking-type', handleExternalSelect);
  }, []);
              ...
              <div className="space-y-4 pt-4 border-t border-white/5">
                ...
                <div className="grid md:grid-cols-2 gap-2">
                  ...
                  >
                    ...
                    <span className="text-[9px] uppercase tracking-[0.2em] font-medium text-left">Non-profit / Community discount request</span>
                  </button>
                  <button 
                    type="button"
          ...
          <div className="p-10 md:p-14 bg-cream/5 border-l border-white/5 flex flex-col justify-end relative">
            <div className="space-y-8 relative z-10">
               <div className="space-y-3 pt-2">
                  <label className="text-[10px] uppercase tracking-[0.4em] opacity-50 font-bold">Your Inquiry</label>
                  <p className="text-base italic text-cream/50 mb-4 font-serif">
                    Tell us about yourself. What are you working on? How may we support you? We will be in touch!
                  </p>
                  <textarea 
                    name="intention"
                    value={formData.message}
                    onChange={(e) => setFormData({ ...formData, message: e.target.value })}
                    placeholder="Shared intentions, context, or specific requests..."
